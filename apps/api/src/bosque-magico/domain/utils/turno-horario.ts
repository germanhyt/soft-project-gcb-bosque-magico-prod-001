import { formatearHorarioServicio, normalizarHora } from './horario-servicio';
...
const HORARIO_TURNO_ESTANDAR: Record<string, { inicio: string; fin: string }> = {
...
export function persistirHorarioDeTurno(
  turno: string,
  inicio?: string | null,
  fin?: string | null,
): { horarioInicio: string | null; horarioFin: string | null } {
  const rango = esTurnoPersonalizado(turno)
    ? rangoTurnoPersonalizado(inicio)
    : resolverHorarioTurno(turno, undefined, { inicio, fin }) ??
      HORARIO_TURNO_ESTANDAR[turno];
  return {
    horarioInicio: rango?.inicio ?? null,
    horarioFin: rango?.fin ?? null,
  };
}
...
function horasSlot(slot: {
  ...
  const hInicio = normalizarHora(slot.inicio);
  const hFin = normalizarHora(slot.fin);
  if (hInicio && hFin) return { inicio: hInicio, fin: hFin };
