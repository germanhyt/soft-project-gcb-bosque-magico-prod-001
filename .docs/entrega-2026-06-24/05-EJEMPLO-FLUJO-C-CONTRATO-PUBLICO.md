...
### Fase 5 — Contrato y pedidos (Por confirmar)

1. **Generar contrato** → enviar → **Marcar enviado**.
2. **Generar desde cotización** → proveedor confirma por link `/pedido/:token`.

### Fase 6 — Confirmar evento

1. **Agenda** → abrir evento → **Confirmar** (requiere pasos anteriores OK).
2. Generar checklist.
...
4. Contrato enviado + proveedor OK → **Confirmar** en Agenda → contrato público en landing.
