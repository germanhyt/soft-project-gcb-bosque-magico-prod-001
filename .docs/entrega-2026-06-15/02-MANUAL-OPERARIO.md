...
| Estado | Significado | Acción |
|--------|-------------|--------|
| **Por confirmar** | Recién creado al aceptar cotización | Revisar datos → **Confirmar evento** |
| **Confirmado** | Fiesta aprobada | Pedidos, checklist, contrato; el día → **Marcar realizado** |
| **Realizado** | Fiesta ya pasó | Archivo operativo |
| **Cancelado** | No se realizará | **Cancelar** con motivo |

### 6.3 Confirmar un evento

1. Busca el evento (filtro **Por confirmar** o calendario).
2. Abre **detalle** (modal).
3. Revisa cliente, fecha, turno, niños.
4. **Confirmar evento** → **Confirmado**.

Al confirmar, si la cotización tiene ítems de **proveedor**, el sistema puede crear pedidos automáticamente.

### 6.4 Pedidos operativos (en detalle del evento)

Sección **Pedidos operativos** — visible cuando el evento está **Confirmado** o **Realizado**.

| Acción | Cuándo usarla |
|--------|---------------|
| **Generar desde cotización** | Crear pedidos a partir de ítems con producto de proveedor |
...
Mientras el evento está **Por confirmar**, verás el mensaje: *Los pedidos se gestionan cuando el evento está confirmado.*
...
En el pie del detalle: **Generar contrato**, enviar WhatsApp, imprimir PDF, marcar enviado/firmado (ver sección 8).
...
3. Cliente acepta → **Agenda** → **Confirmar** → **Generar desde cotización** → checklist → contrato.
...
2. **Agenda** → confirmar → pedidos + contrato.
...
3. **Confirmar evento** → **Generar desde cotización** → aparece pedido en Agenda y en **Operaciones**.
...
1. **Agenda** → **Confirmado** → **Marcar realizado**.
...
| Pruebas sandbox | `.docs/entrega-2026-06-15/03-PRUEBAS-Y-QA.md` |
| Flujos con diagramas | `.docs/BOSQUE_FLUJOS_Y_GUIA_USO.md` |

---

*Manual operativo — Bosque Mágico. Versión 1.2 — 2026-06-15.*
